...
exports.createBooking = async (req, res) => {
    ...
    try {
        ...
        // Gerar código de operação para a reserva
        const operationCode = await registerOperation('bookings', bookingId, connection);
        await connection.query(
            'UPDATE bookings SET operation_code = ? WHERE id = ?',
            [operationCode, bookingId]
        );
...
// Buscar reserva por código de operação
exports.getBookingByCode = async (req, res) => {
    try {
        const { code } = req.params;

        const [bookings] = await pool.query(`
            SELECT b.*, g.name as guest_name, r.room_number, rt.name as room_type
            FROM bookings b
            JOIN guests g ON b.guest_id = g.id
            JOIN rooms r ON b.room_id = r.id
            JOIN room_types rt ON r.room_type_id = rt.id
            WHERE b.operation_code = ?
        `, [code]);
        ...
        console.error('Erro ao buscar reserva por código:', error);
        res.status(500).json({ error: 'Erro ao buscar reserva' });
    }
};    
